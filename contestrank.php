...
for ($i=0;$i<$rows_cnt;$i++){
        ...
        if (strcmp($user_name,$n_user)){
                ...
                $U[$user_cnt]->nick=$row['school']."_".$row['nick'];
                $user_name=$n_user;
        }
        if(time()<$end_time+18000&&$lock<strtotime($row['in_date']))
...
if($OJ_MEMCACHE){
	$sql="select num,user_id from solution where contest_id=$cid and result=4 order by solution_id ";
        $fb = mysql_query_cache($sql);
        if($fb) $rows_cnt=count($fb);
        else $rows_cnt=0;
}else{
	$sql="select num,user_id from solution where contest_id=? and result=4 order by solution_id ";
