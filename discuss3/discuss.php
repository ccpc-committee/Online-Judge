...
if ($prob_exist){?>
		...
		<span style='font-size:14px' >[</span> <a style='font-size:14px' href="newpost.php<?php
		if ($pid!=0 && $cid!=null) 
			...
			echo "?cid=".$cid;?>
		">New Thread</a> <span style='font-size:14px' >]</span></div>
		<div style="float:left;text-align:left;font-size:80%">
		<span style='font-size:14px' >Location :&nbsp;&nbsp;</span>
		<?php if ($cid!=null) echo "<a style='font-size:14px;margin-top:5px' href=\"discuss.php?cid=".$cid."\">Contest ".$cid."</a>"; else echo "<a href=\"discuss.php\">MainBoard</a>";
		if ($pid!=null && $pid!=0){
				$query="?pid=$pid";
				if($cid!=0) {
					$query.="&cid=$cid";
					$PAL=pdo_query("select num from contest_problem where contest_id=? and problem_id=?",$cid,$pid)[0][0];
					 echo " >> <a style='font-size:14px' href=\"discuss.php".$query."\">Problem ".$PID[$PAL]."</a>";
				}else{
